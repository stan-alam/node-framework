...
//function encodeMe(callback) {
        ...
        fs.readFile('./sessionStorage.txt', 'utf8', function(err, sessionStorage) {
            ...
            console.log('\nSession Storage from file used for use in JWT: ' + sessionStorage);
            ...
            fs.writeFile("./sessionSpecialStorage.txt", encoded, function(err) {
                if (err) {
                    return console.log(err);
                }
            });

            fs.writeFile("./tenantID.txt", decoded.tenant.id, function(err) {
                if (err) {
                    return console.log(err);
                }
            });
