...
function openJwt(jwtToken, callback){
      jwt.verify(jwtToken, privateKey, function(error, decoded){
...
function getAccessToken(ApiKey, callback){
    ...
    if(res.statusCode != 200){
        ...
        jwt.verify(returnJwt, privateKey, function(error, decoded){
                                                callback(error, returnJwt)
                                              });
    }
}
...
/**
* Call to convert an application apikey to an Access Token Using Bad Method
* @async
*
* @method getAccessTokenBadMethod
* @param {Guid} ApiKey - Application ApiKey from application configuration
* @return {String} Returns Encrypted and signed JWT from auth service
*/
function getAccessTokenBadMethod(ApiKey, callback){
    let url = envVars.url+envVars.global.tokenService;
...
/**
* Call to convert an application apikey to an Access Token No Header
* @async
*
* @method getAccessTokenNoHeader
* @param {Guid} ApiKey - Application ApiKey from application configuration
* @return {String} Returns Encrypted and signed JWT from auth service
*/
function getAccessTokenNoHeader(ApiKey, callback){
    let url = envVars.url+envVars.global.tokenService;
    let res = request('POST', url, {
        'headers': {
        }
    });
    callback(null, {'body': res.body.toString('utf8'), 'statusCode': res.statusCode})
}
/**
...
*/
function getEPAtoken(jwt, callback){
    openJwt(jwt, function(error,contentsToken){
       if(!error){
...
* @method expireToken
* @param {String} jwt - Contents of JWT
* @return {String} Encrypted and signed JWT that is expired
*/
function expireToken(jwt, callback){
    openJwt(jwt, function(error,contentsToken){
       if(!error){
           if(contentsToken.exp){
               contentsToken.iat = contentsToken.exp+10;
           }else{
                contentsToken.exp = contentsToken.iat-10;
           }
           contentsToken.keepIat = true;
           closeJwt(contentsToken, function(jwt){
              callback(null, jwt);
           });
        }
...
module.exports = exports = {
                        ...
                        getEPAtoken: getEPAtoken,
                        expireToken: expireToken,
                        getAccessTokenNoHeader: getAccessTokenNoHeader,
                        getAccessTokenBadMethod: getAccessTokenBadMethod
                        };