...
function addCredentials(appID,jwt,callback){
 authFunctions.openJwt(jwt, function(token){
    console.log("posting credentials");
      let credentialsBody = {
                 "id":"670eb5ed-7c00-4cba-bcfa-0d16a1b1d73a","title":"EPA credentials","credentials":[{"application":{"id":appID},"password":"epapassword","username":"epausername"}]
            };
            let res = request('POST', envVars.test.url+"/admin/"+token.tenant.alias+"/api/privileged-app-credentials", {
                'headers': {
            ...
            if(res.statusCode !== 201){
                console.log("Config Insert Credentials Failed : "+JSON.stringify(res.getBody('utf8')));
                callback(null,"Config Insert Credentials Failed");
            }else{
                  console.log("Post credentials successful.Response body:" +  JSON.stringify(res.getBody('utf8'))); 
