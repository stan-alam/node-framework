...
/**
 * Call to delete one EPA Configuration
 * @async
 *
 * @method deleteEpaConfig
 * @param {guid} id - Configuration EPA ID to be deleted
 * @param {string} JWT - Token to be used when making calls to config
 * @return {object} Objects containing statusCode
 */
function deleteEpaConfig(id, jwt, callback){
    authFunctions.openJwt(jwt, function(error,token){
        let res = request('DELETE', envVars.url+"/admin/"+token.tenant.alias+"/api/privileged-app-credentials/"+id, {
            'headers': {
                "content-type": envVars.headers.privilegedappcredentials,
                "Authorization": "Bearer "+ jwt
            }
 ...
 * @async
 *
 * @method getEpaConfiguration
 * @param {string} JWT - Token to be used when making calls to config
 * @return {array} Array of applications from configuration
 */
function getEpaConfiguration(jwt, callback){
    authFunctions.openJwt(jwt, function(error,token){
        let res = request('GET', envVars.url+"/admin/"+token.tenant.alias+"/api/privileged-app-credentials", {
                    'headers': {
                        "content-type": envVars.headers.privilegedappcredentials,
                        "Authorization": "Bearer "+ jwt
                    }
        ...
        if(res.statusCode !== 200){
            callback({ 'msg': "Config reading EPA Configurations Failed", "statusCode": res.statusCode });
        }else{
            callback(null, JSON.parse(res.getBody('utf8')));
        }
    });
 ...
 /**
  * Call to add credentials to an application
  * @async
  *
  * @method addCredentials
  * @param {string} JWT - Token to be used when making calls to config
  * @param {object} application - Application Object to add Resource as subscription
  * @param {string/array} credentials - array of credentials
  * @return {array} Array of Objects for each response body and statusCode
  */
function addCredentials(jwt, application, credentials, callback){
    authFunctions.openJwt(jwt, function(error,token){
        delete application['metadata'];
        if (application['credentials'] === undefined) {
            application['credentials'] = [];
        }
        _.each(credentials,function(cred){
            delete cred['applicationName'];
            application['credentials'].push(cred);
        });

        let res = request('PUT', envVars.url + "/admin/" + token.tenant.alias + "/api/applications/" + application.id, {
            "headers": {
                "content-type": envVars.headers.config,
                "Authorization": "Bearer "+ jwt
                },
        ...
        if(res.statusCode !== 200){
            callback({ 'msg': "Config adding credentials Failed", 'statusCode': res.statusCode });
        }else{
            callback(null, {'body': JSON.parse(res.getBody('utf8')), 'statusCode': res.statusCode });
        }
    });
...
module.exports = exports = {
    ...
    deleteEpaConfig: deleteEpaConfig,
    getEpaConfiguration: getEpaConfiguration,
