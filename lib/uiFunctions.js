...
function uiAction(driver, options, callback){
    ...
    if (options.hoverXpath) {
        ...
        driver.wait(until.elementLocated(By.xpath(options.hoverXpath.toString())), 25000)
    ...
    let errorUI = function(err){
         if (err.state && err.state === 'no such element') {
            if(options.allowFail){
                callback(driver, {});
            }
        } else {
            callback(driver, null, err)
        }
    }

    if(options.action == 'click'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).click().then(function(){
                callback(driver, {});
       }, function(err){ errorUI(err) });
   }else if(options.action == 'elementView'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).getInnerHtml().then(function(field){
                callback(driver, { 'text': field });
       });
   }else if(options.action == 'clear'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).clear().then(function(){
                callback(driver, { });
       });
   }else if(options.action == 'elementHtml'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).getOuterHtml().then(function(element){
           callback(driver, { 'text': element });
       }, function(err){ errorUI(err) });
   }else if(options.action == 'elementGetAttribute'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).getAttribute(options.attributeValue).then(function(element){
           callback(driver, { 'text': element });
       }, function(err){ errorUI(err) });
   }else if(options.action == 'sendkeys'){
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 25000).then(function(ele){
        ele.sendKeys(options.elementValue).then(function(){
                callback(driver, {});
         }, function(err){ errorUI(err) });
       }, function(err){ errorUI(err) });
   }else if(options.action == 'sendSpecialKey'){
       let keyfunc = eval('module.exports = function () { return webdriver.Key.'+ options.key +'} ');
       driver.wait(until.elementLocated(By.xpath(options.xpath.toString())), 2500000).then(function(ele){
        ele.sendKeys(keyfunc()).then(function(){
                callback(driver, {});
         }, function(err){ errorUI(err) });
       }, function(err){ errorUI(err) });
